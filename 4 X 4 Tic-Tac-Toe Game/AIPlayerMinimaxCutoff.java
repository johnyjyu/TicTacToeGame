 ...
 * Eval(s) = 6x_3 + 3x_2 + x_1 -(6o_3 + 3o_2 + o_1)
 */
public class AIPlayerMinimaxCutoff extends AbstractAIPlayer {
	private boolean cutoffOccurred = false;
	private int maxDepth = 0;
	private int totalNumNodes = 0;
	private int numPrunMax = 0;
	private int numPrunMin = 0;
   private int HIGHEST_DIFFICULT_LEVEL = 9; // chosen to make sure max player choose move in 10 seconds
   private int depthLimit; // depth searched by computer
 
   /** Constructor with the given game board */
   public AIPlayerMinimaxCutoff(Board board, int difficultLevel) {
      super(board);
      int depthLimit = HIGHEST_DIFFICULT_LEVEL-(8-4*difficultLevel); // set search depth cutoff limit
      this.depthLimit = depthLimit;
      setSeed(Seed.CROSS);  // set AI as CROSS and human as NOUGHT
   }
   ...
   int[] move() {
      ...
      int[] result = minimax(depthLimit, depthLimit, mySeed, -1000, 1000); //parameters: depthLimit, depth, turn, alpha, beta
      long endTime = System.nanoTime();  // record run time for a move by AIPlayer
      long duration = (endTime - startTime)/100000000;  //divide by 1000000 00 to get 100 milliseconds.

      // output info on screen after each move by AIPlayer
   ...
   private int[] minimax(int depthLimit, int depth, Seed player, int alpha, int beta) {
      // Generate all possible next moves in a List of int[2] of {row, col}.
   ...
   /** The heuristic evaluation function for the current board2
   		where x_n is number of rows, colums, diagoals with exactly n Xs and no Os
   		   	O_n ...                                                Os....     Xs
       @Return +1000, +6, +3, +1 for EACH 4-, 3-, 2-, 1-in-a-line for computer and non-in-the-same-line for oppoent.
               -1000, -6, -3, -1 for EACH 4-, 3-, 2-, 1-in-a-line for opponent and non-in-the-same-line for computer.
               0 otherwise   */
